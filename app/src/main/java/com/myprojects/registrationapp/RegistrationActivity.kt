...
class RegistrationActivity : AppCompatActivity() {
    ...
    private fun validationResult(): Boolean {
        if (!nameInputLayout.isErrorEnabled && !nameEt.text.isEmpty() &&
            !surnameInputLayout.isErrorEnabled && !surnameEt.text.isEmpty() &&
            !ageInputLayout.isErrorEnabled && !ageEt.text.isEmpty() &&
            !emailInputLayout.isErrorEnabled && !emailEt.text.isEmpty() &&
            !passwordInputLayout.isErrorEnabled && !passwordEt.text.isEmpty() &&
            !mobileInputLayout.isErrorEnabled && !mobileEt.text.isEmpty()
        ) {
            return true
        }
        return false
    ...
    private inline fun validateInput(inputLayout: TextInputLayout, inputView: EditText, crossinline body: () -> Unit): Disposable {
    ...
    private fun validateFields() {
        with(disposable) {
            ...
            add(validateInput(mobileInputLayout, mobileEt) { isMobileValid(mobileEt.text.toString()) })
    ...
    private fun isMobileValid(mobile: String) {
        if (mobile.isEmpty() ) {
            mobileInputLayout.isErrorEnabled = true
            mobileInputLayout.error = getString(R.string.field_validation_error)
        } else if (mobile.toInt() < 11) {
            mobileInputLayout.isErrorEnabled = true
            mobileInputLayout.error = getString(R.string.mobile_validation_error)
        } else {
            mobileInputLayout.isErrorEnabled = false
        }
    }
